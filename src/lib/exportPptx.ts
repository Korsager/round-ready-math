...
export function exportPptx(a: Assumptions, pricingArg?: PricingStrategy, charts?: ExportCharts) {
  ...
  // Summary — four-question structure
  const annualGrowthPct = (Math.pow(1 + a.forecast.monthlyGrowthRate / 100, 12) - 1) * 100;
  const aligned = implied.impliedIrrPct >= a.fundraise.targetIrr;
  const verdict = aligned
    ? "Aligned — forecast supports investor IRR."
    : (implied.impliedIrrPct >= a.fundraise.targetIrr * 0.7
        ? "Gap — forecast falls short of investor IRR."
        : "Critical — story does not yet support investor IRR.");

  const sum = pres.addSlide();
  sum.background = { color: NAVY };
  sum.addText("Summary", { x: 0.6, y: 0.4, w: 12, h: 0.4, fontSize: 14, bold: true, color: ACCENT, charSpacing: 4 });
  sum.addText(`Raising ${fmtM(a.fundraise.raise)} at ${fmtM(postMoney)} post-money`, {
    x: 0.6, y: 0.85, w: 12, h: 0.7, fontSize: 28, bold: true, color: "FFFFFF", fontFace: "Calibri",
  });

  const line = (n: number, y: number, lead: string, body: string) => {
    sum.addText(`${n}.`, { x: 0.6, y, w: 0.5, h: 0.4, fontSize: 18, bold: true, color: ACCENT, fontFace: "Calibri" });
    sum.addText(lead, { x: 1.1, y, w: 11.5, h: 0.4, fontSize: 16, bold: true, color: "FFFFFF", fontFace: "Calibri" });
    sum.addText(body, { x: 1.1, y: y + 0.4, w: 11.5, h: 0.7, fontSize: 13, color: ICE, fontFace: "Calibri" });
  };

  line(1, 1.85, `Investors expect ${a.fundraise.targetIrr}% IRR.`,
    `Your forecast supports ${implied.impliedIrrPct.toFixed(1)}% at ${annualGrowthPct.toFixed(0)}% annualized revenue growth.`);
  line(2, 3.05, `Runway: ${cf.runwayMonth ? `${cf.runwayMonth} months` : "36+ months"} today.`,
    `After the current raise of ${fmtM(a.fundraise.raise)}: ${cf.monthsRunwayAfterRaise === null ? "—" : `${cf.monthsRunwayAfterRaise} months`}.`);
  line(3, 4.25, `Cash at month 36: ${fmtUsd(cf.endingCash)}.`,
    `Rounds needed: ${cf.plannedRaises.filter((r) => r.month <= 36 && r.amount > 0).length} totaling ${fmtM(cf.totalRaisedTo36)}. Founder ownership after all rounds: ${cf.founderOwnershipAtM36.toFixed(0)}%.`);
  line(4, 5.45, `Verdict: ${verdict}`,
    aligned
      ? `IRR, runway, and capital plan all line up.`
      : `Driven by ${implied.impliedIrrPct < a.fundraise.targetIrr ? "the forecast-implied IRR gap" : "runway pressure"}.`);

  sum.addText("Generated by the Founders Toolkit", { x: 0.6, y: 6.95, w: 12, h: 0.3, fontSize: 11, color: ICE, italic: true });
